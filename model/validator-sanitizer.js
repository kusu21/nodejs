 ...
 *
 * @param location
 * @param dob in FSJ date format i.e. "YYYY-MM-DD".
 *
 * @returns validation chain object of express-validator
 */
function isDOB(location, dob) {
  ...
  return validator(dob)
    .custom((paramDOB) => {
      ...
      if (hf.isFSJDateFormat(paramDOB) && paramDOB >= '1940-01-01') {
        return true;
      }
      return false;
    })
...
/**
 * Date validator to check if the date provided is of correct FSJ date format (YYYY-MM-DD)
 *
 * @param location
 * @param date Format "3012-04-23". This is FSJ Date format "YYYY-MM-DD"
 *
 * @returns validation chain object of express-validator
 */
function isValidFSJDate(location, date) {
  const validator = getFunctionName(location);
  return validator(date)
    .custom((paramDate) => {
      if (hf.isFSJDateFormat(paramDate)) {
        return true;
      }
      return false;
    })
...
/**
 * Time validator to check if the time provided is in correct FSJ time format (HH:mm:ss)
 *
 * @param location
 * @param time Format "20:30:05". This is FSJ Time format "HH:mm:ss"
 *
 * @returns validation chain object of express-validator
 */
function isValidFSJTime(location, time) {
  const validator = getFunctionName(location);
  return validator(time)
    .custom((paramTime) => {
      if (hf.isFSJTimeFormat(paramTime)) {
        return true;
      }
      return false;
    })
...
function isValidFutureDateTodayNotAllowed(location, date) {
  ...
  return validator(date)
    .custom((paramDate) => {
      if (!hf.isFSJDateFormat(paramDate) || paramDate <= hf.getFSJFormatToday()) {
        return false;
      }
...
module.exports.isValidFSJDate = isValidFSJDate;
module.exports.isValidFSJTime = isValidFSJTime;
